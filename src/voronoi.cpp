...
Voronoi::Voronoi(float min_x_pos, float max_x_pos, float min_y_pos, float max_y_pos, float padding) {
    ...
    m_cell.push_back(raylib::Vector2(min_x_pos + padding, min_y_pos + padding));
    m_cell.push_back(raylib::Vector2(max_x_pos + padding, min_y_pos + padding));
    m_cell.push_back(raylib::Vector2(max_x_pos + padding, max_y_pos - padding));
    m_cell.push_back(raylib::Vector2(min_x_pos - padding, max_y_pos - padding));
    m_padding = padding;
}
...
void Voronoi::generate_random(uint16_t amount_sites) {
    ...
    for(size_t i = 0; i < amount_sites; i++) {
        ...
        float random_x = generate_random_number(m_cell.at(TL_INDEX).x + m_padding, m_cell.at(TR_INDEX).x - m_padding);
        float random_y = generate_random_number(m_cell.at(TL_INDEX).y, m_cell.at(BL_INDEX).y - m_padding);
...
int Voronoi::generate() {
    ...
    for(auto& current_site : m_sites) {
        ...
        auto cell = m_cell;
        std::vector<raylib::Vector2> new_cell;
        ...
        for(auto& neighbor_site : m_sites) {
            ...
            raylib::Vector3 bisector = calculate_bisector(current_site, neighbor_site);
            ...
            // finde die intersections zur aktuellen zelle
            for(size_t i = 0; i < cell.size(); i++) {
                ...
                if(first_intersection.has_value()) {
                    ...
                    new_cell.push_back(first_intersection.value());
                    new_cell.push_back(next_cell);
            ...
            if(new_cell.size() == 0) {
                ...
                if(!point_in_polygon(current_site, new_cell)) {
                    ...
                    for(size_t k = second_intersection_start_index; k % cell_size > first_intersection_index || k % cell_size < first_intersection_index; k++) {
                        ...
                        new_cell.push_back(v1);
                    }

                    if(first_intersection != v1) new_cell.push_back(first_intersection.value());
        ...
        if(!cell.empty()) {
            m_voronoi_cells.push_back(new_cell);
        }
...
std::optional<raylib::Vector2> Voronoi::line_segment_intersection(raylib::Vector3 line, raylib::Vector2 point_A, raylib::Vector2 point_B) {
    ...
    raylib::Vector2 intersection(p.x / p.z, p.y / p.z); // homogen(3d) -> heterogen(2d)
...
std::vector<raylib::Vector2>& Voronoi::getBBBox() {
    ...
    //raylib::Rectangle rec(m_cell.x + m_padding, m_cell.y + m_padding, m_cell.width -  2 * m_padding,
     //   m_cell.height - 2 * m_padding);
    return m_cell;
}
